...
\begin{equation}
    \theta^\star = \arg \min_{\theta} \underbrace{\sum_{i=1}^{N}{\lVert \Psi_i - \mathcal{H}_i \circ \mathcal{M}_{\theta}\left( t_i\right)\rVert}^2_{\mat{R}^{-1}}}_{\displaystyle \loss{obs}\left(\theta\right)} + \underbrace{{\lVert \theta - \theta^b \rVert}^2_{\mat{B}^{-1}}}_{\displaystyle \loss{reg}\left(\theta\right)} ,
\end{equation}

where $\mat{R}$ and $\mat{B}$ are the observation error covariance and the background error covariance, respectively, wehreas $\theta^b$ (called the background) is a prior estimate of the control vector. The background term $\loss{reg}$ regularizes the optimization problem by penalizing the deviation of $\theta$ from $\theta^b$.

Retrieving a relevant estimation for $\theta^b$ can already be a challenge of itself and should not be overlooked. Indeed, when performing data assimilation using multilayer QG models, it is often difficult to obtain a precise \textit{a priori} estimation for the subsurface flow that can be used as a background term.   Since this background term is solely there to ensure the well-posedness of the minimization problem, the background regularization can be replaced by any cost function that would enforce an \textit{a priori} knowledge on the dynamics. For instance, penalizing the stream function's second derivatives from deviating from 0 has been proven to be effective in regularizing the assimilation problem as it enforces the smoothness of the fields~\cite{vogeler1995,cong1995}.
