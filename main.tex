...
In the present study, we rewrite the top layer equation of the multilayer QG framework to derive a truncation\nobreak -\nobreak aware RG equation in~\cref{sec:rg-si-model}. This formulation is based on the introduction of an additional forcing composed of the sublayer stream function's material derivative. VA is then performed to find the sublayer field that allows for the most accurate reconstruction of the surface flow. Additionally, in~\cref{sec:deformation-radius-correction}, we introduce a single-parameter deformation radius correction to help the model handle incorrect stratification parameters. As our method provides a direct estimation for the sublayer stream function, we are also able to replace the usual background term of the cost function by a physics-based regularization enforcing PV conservation in the sublayer (\cref{sec:reg}).
In order to validate our method, we perform two experiments. In~\cref{sec:qg3l}, we first use a three-layer QG double-gyre simulation as reference to monitor truncation-only error mitigation and discuss the relevancy of the parameterized field as a proxy for the sublayer stream function. Then, in~\cref{sec:enatl60}, we use a primitive equation simulation to assess the model's reconstruction capabilities of a flow with richer dynamics.
...
As this work focuses on introducing a new parameterization to reduce model error, we have decided to only consider such parameterization in the control vector. Ideal initial condition (IC) and boundary conditions (BC) are therefore retrieved from the surface flow of the reference simulation. Since all tested frameworks (RG0, RG-F, RG-SI) share identical IC and BC, differences in performances only result from model error and its correction. Although IC and BC can be included in the control vector to further extend the scope of this work, we have decided not to do so in the present study as the estimation of IC and BC through variational assimilation is already a field of study of its own~\cite{cong1998,zou1995,gunson1996,seiler1993,ayoub2006}.
