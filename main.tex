...
\affiliation{1}{Centre INRIA de l'Université de Rennes, IRMAR, ODYSSEY, Rennes, France} % First argument is their footnote number, as above.
...
\authoraddr{%
    ...
    Gaétan Rigaut, gaetan.rigaut@inria.fr
}
...
% Key points.  
%   Takes three arguments.  Remember to specify them all, using {} for no item.
%
\keypoints%
    {Enhancing reduced-gravity model reconstruction in the context of data assimilation using a theory-motivated model error parameterization}
    {Introducing a physics-derived regularization term for the cost function to enforce multilayer QG dynamics}
    {Improving the robustness of the model by introducing a deformation radius correcting parameter in the control vector}
...
\begin{abstract}
    Ocean surface reconstruction through variational assimilation (VA) of partial, satellite-acquired observations is often performed using reduced-order models. 
    The quasi-geostrophic reduced-gravity (RG) framework, a truncation of the multilayer quasi-geostrophic equations, has shown promising reconstructing capabilities.
    In order to address model errors, a state-of-the-art reconstruction method includes an external forcing in the advection equations of the RG model and optimizes this term during the VA procedure.
    ...
    Since this method yields a proxy for the subsurface stream function, we can compute an approximation of the subsurface potential vorticity and an estimation of its conservation.
    ...
    Validation is performed using two different reference datasets, a three-layer quasi-geostrophic double-gyre simulation and the eNATL60 simulation, both with observations retrieved by an OSSE along SWOT-like tracks.
    Results show a reduction of normalized RMSE by 0.12 and 0.05 on surface stream function reconstruction error in each experiment (respectively) when compared to a RG model corrected by an external forcing.
    ...
    \begin{plainlanguagesummary}
        Knowing the ocean surface state is essential for many applications, among which tracking marine currents or predicting weather patterns. While satellites provide the vast majority of measurements of the ocean, they only capture surface fields, and do not ensure global coverage at all time. To fill the gaps, observations are integrated into numerical simulations that are usually performed using simplified models of ocean dynamics. State-of-the-art reconstruction methods are built upon the reduced-gravity quasi-geostrophic model, which is a simplification of a more extensive framework, the multilayer quasi-geostrophic system. The simplification is made by considering all the deep ocean flow to be at rest. In this study we bridge the gap between the two frameworks by introducing a method that infers the subsurface flow in order to better emulate surface dynamics. When tested against numerical simulations, our method shows a significant reduction of the reconstruction error, both when assessed in a simplified setting and a high-resolution realistic setting (error reduced by 0.12 and 0.05, respectively). These results demonstrate the importance of using theoretical knowledge of the ocean dynamics to help reconstruction models account for their inherent sources of error.
...
Around the mid-latitudes, a simplified framework of choice is the quasi-geostrophic (QG) theory. This framework, developed in the 1950s~\cite{charney1950,charney1953}, describes the behavior of flows around the geostrophic balance through the material conservation of potential vorticity (PV) by the geostrophic flow. Such theory has paved the way for many simplified numerical models, whether atmospheric or oceanic~\cite{phillips1951,held1995}. Although it describes 3D dynamics, it can be discretized by stacking layers of fluid with constant vertical properties, yielding the multilayer QG framework~\cite{vallis2006}. 

Studies in the late 1980s used QG systems to perform data assimilation. Some methods consisted of updating the surface state with observations and then allowing the model's dynamics to adjust to the observation\nobreak{}-\nobreak{}imposed circulation~\cite{haines1991}. However, to produce satisfactory reconstructions, these methods required extensive spin-up times and therefore significant amount of data. Other methods focused on using statistical information on the flow dynamics to project surface observations onto empirical vertical modes~\cite{mey1987}. Although promising, these methods required many subsurface observations to infer reliable statistics thus preventing their application to the wide ocean.

Among DA methods, variational assimilation (VA) has been successfully employed to reduce discrepancies between observations and numerical simulations~\cite{gunson1996,cong1998,zou1995,vogeler1995,vogeler1999}. VA consists in performing successive iterations of a numerical model, from which reconstruction error is assessed. The backpropagation of the error using the adjoint model of the numerical code~\cite{errico1997} then allows refining the control parameters (initial condition for example) from an \textit{a priori} estimation to reduce such error. Despite the (significant) computation cost of determining and storing the adjoint model, VA is still widely used in operational assimilation software~\cite{martin2025}. Performing VA using a multilayer QG simulation has been widely investigated in the late 1980s~\cite{cong1998,vogeler1995,vogeler1999}. But it has been pointed out~\cite{vogeler1995,vogeler1999,hurlburt1986,morrow1995} that, in the absence of direct subsurface observations, an accurate first guess of the initial condition was required to further refine surface dynamics reconstruction.
...
In the present study, we rewrite the top layer equation of the multilayer QG framework to derive a truncation\nobreak -\nobreak aware RG equation in~\cref{sec:rg-si-model}. This formulation is based on the introduction of an additional forcing composed of the sublayer stream function's material derivative. VA is then performed to find the sublayer field that allows for the most accurate reconstruction of the surface flow. Additionally, we introduce a single-parameter deformation radius correction to help the model handle incorrect stratification parameters. As our method provides a direct estimation for the sublayer stream function, we are also able to replace the usual background term of the cost function by a physics-based regularization enforcing PV conservation in the sublayer. The resulting variational assimilation framework is described in~\cref{sec:va-framework}.
In order to validate our method, we perform two experiments, that we detail in~\cref{sec:method-validation}. We first use a three-layer QG double-gyre simulation as reference to monitor truncation-only error mitigation and discuss the relevancy of the parameterized field as a proxy for the sublayer stream function. Then, we use a primitive equation simulation to assess the model's reconstruction capabilities of a flow with richer dynamics.


\section{Reduced-gravity QG model with parameterized sublayer flow}\label{sec:rg-si-model}
...
\begin{equation}
    \mat{A} = \begin{bmatrix}
        \left( \frac{1}{H_1 g_1} + \frac{1}{H_1 g_2} \right) & -\frac{1}{H_1 g_2} & \makebox[1.5cm]{$.$} & \makebox[1.5cm]{$.$} \\[0.5em]
        -\frac{1}{H_2 g_2} & \left( \frac{1}{H_2 g_2} + \frac{1}{H_2 g_3} \right) & -\frac{1}{H_2 g_3} & . \\[0.5em]
        . & \cdots & \cdots & \cdots \\[0.3em]
        . & . & -\frac{1}{H_N g_N} & \frac{1}{H_N g_N}
    \end{bmatrix}.
\end{equation}

One sees that the different layers are coupled with each other via the vortex stretching term in the equation for the PV. 
...
Accordingly, the reduced-gravity model as expressed in~\cref{eq:qg-rg} was used to perform dynamical interpolation~\cite{ubelmann2015,ubelmann2016}. 
Benefiting from the ability of the QG model to generate non-linear dynamics, such method has been able to outperform optimal interpolation. 
However, due to the truncated QG equations, significant errors can emerge as pointed out in~\citeA{manucharyan2021}. 
In order to address this limitation, the VarDyn framework~\cite{leguillou2023,leguillou2025} proposed to take into account model errors by introducing an infinitesimal external forcing, $\epsilon$, into the advection equation,
\begin{equation}
    \pdvt{\pv^{RG}} + J\left(\sf_1, \pv^{RG}\right) = \epsilon.
\end{equation}

We will use RG-F (Reduced Gravity - Forced) to denote our implementation of this method in the following. Details on the parameterization of $\epsilon$ on a reduced-order space are given in~\ref{appendix:rg-f-implementation}.

Here, we propose a model for the error term by assuming that it represents directly the influence of the lower-layer dynamics on the surface layer, following the derivation of the reduced-gravity QG equations presented above. 
Indeed, from the definition of the QG PV, the PV in the upper layer can be written
\begin{equation}
    \pv_1 = \pv^{RG} + \frac{f_0^2}{H_1 g_2} \sf_2.
\end{equation}
Using this decomposition into the PV upper layer advection equation~\eqref{def:pv-adv} yields
\begin{eqnarray}\label{def:qg-rg-adv-corr}
...
\end{eqnarray}
From this expression, we note that the usual RG model advection matches this multilayer formulation when $\Dt*{\sf_2}=0$. 
...
\begin{equation}
    ...
    \quad q_2 = \lap{\tilde{\sf}_2} - \frac{f_0^2}{H_2 g_2}\left(\tilde{\sf}_2 - \sf_1\right) + \frac{f_0^2}{H_2 g_3}\left(\sf_3 - \tilde{\sf}_2\right)
\end{equation}
...
\begin{eqnarray}
    \tilde{\mat{A}}\left(\alpha\right) & = & \mat{C}_{\mathrm{M2L}}\begin{bmatrix}\left(1+\alpha\right)\lambda_{\rightleftarrows} & 0 \\ 0 & \lambda_{\rightrightarrows}\end{bmatrix}\mat{C}_{\mathrm{L2M}}, \\
    && = \mat{A} + \alpha\lambda_{\rightleftarrows}\mat{C}_{\mathrm{M2L}}\begin{bmatrix}1 & 0 \\ 0 & 0\end{bmatrix}\mat{C}_{\mathrm{L2M}},
\end{eqnarray}

which coincides with the default stretching matrix when $\alpha=0$. Although $\tilde{\mat{A}}\left(\alpha\right)$ is a function of $\alpha$, in the following, we will simply write $\tilde{\mat{A}}$. The barotropic deformation radius then remains unchanged as it is linked to the total depth which can be quite easily approximated whereas the baroclinic deformation radius, $L_{\rightleftarrows}$ becomes

\begin{equation}
    L_{\rightleftarrows} = \frac{1}{f_0 \sqrt{\left(1+\alpha\right)\lambda_{\rightleftarrows}}}.
\end{equation}

Our model implementation replaces $\mat{A}$ by $\tilde{\mat{A}}$ in the usual two-layer QG equations and the parameter $\alpha$ is added in the control vector, granting one additional degree of freedom for the variational assimilation. Denoting $\tilde{\mat{A}}_{i,j}$ the $i$-th row, $j$-th column of $\tilde{\mat{A}}$, the final formulation of the RG-SI model equations are therefore

\begin{eqnarray}\label{eq:rg-si}
    {\pv}^{RG} & = & \lap{\sf_1} - f_0^2 \tilde{\mat{A}}_{1,1} \sf_1, \\
    \pdvt{\pv^{RG}} + J\left(\sf_1, \pv^{RG}\right) & = & f_0^2 \tilde{\mat{A}}_{1,2}\Dt{\tilde{\sf}_2} \\
    \tilde{\pv}_2 & = & \lap{\tilde{\sf}_2} - f_0^2 \left(\tilde{\mat{A}}_{2,1} \sf_1 + \tilde{\mat{A}}_{2,2} \tilde{\sf}_2\right).
\end{eqnarray}

\section{Variational assimilation of altimeter data with the RG-SI model}\label{sec:va-framework}
...
Provided a set of (partial) observations of the surface stream function $\left( \Psi_i \right)$ retrieved at times $\left( t_i \right)$, $i \in \{1, \dots, N\}$, we emulate the surface state using a model $\mathcal{M}_{\theta}$ (RG-SI for example), which depends on the control vector $\theta$. The latter may include any parameter the propagator dynamics depends on (initial and boundary conditions, model parameters\dots). Estimated states are then processed through the observation operator $\mathcal{H}$ in order to be compared to the observations. Finally, variational assimilation reduces to finding $\theta^\star$ such that it minimizes the discrepancy between the model and observations through a cost function function gathering a time varying discrepancy term and a regularization term on the control vector:


\begin{equation}
    \theta^\star = \arg \min_{\theta} \underbrace{\sum_{i=1}^{N}{\lVert \Psi_i - \mathcal{H}_i \circ \mathcal{M}_{\theta}\left( t_i\right)\rVert}^2_{\mat{R}^{-1}}}_{\displaystyle \loss{obs}\left(\theta\right)} + \underbrace{{\lVert \theta - \theta^b \rVert}^2_{\mat{B}^{-1}}\vphantom{\sum_{i=1}^{N}{\lVert \Psi_i - \mathcal{H}_i \circ \mathcal{M}_{\theta}\left( t_i\right)\rVert}^2_{\mat{R}^{-1}}}}_{\displaystyle \loss{reg}\left(\theta\right)} ,
\end{equation}

where $\mat{R}$ and $\mat{B}$ are the observation error covariance and the background error covariance, respectively, whereas $\theta^b$ (called the background) is a prior estimate of the control vector. The background term $\loss{reg}$ regularizes the optimization problem by penalizing the deviation of $\theta$ from $\theta^b$.

Retrieving a relevant estimation for $\theta^b$ can already be a challenge in itself and should not be overlooked. Indeed, when performing data assimilation using multilayer QG models, it is often difficult to obtain a precise \textit{a priori} estimation for the subsurface flow that can be used as a background term. Since this term is solely there to ensure the well-posedness of the minimization problem, the background regularization can be replaced by any cost function that would enforce an \textit{a priori} knowledge on the dynamics. For instance, penalizing the stream function's second derivatives from deviating from 0 has been proven to be effective in regularizing the assimilation problem as it enforces the smoothness of the fields~\cite{vogeler1995,cong1995}.

However, this does not replace the need for an actual initial value $\theta^0$ for $\theta$ to start assimilating from. To overcome this difficulty,~\citeA{cong1998} proposed to use a preparation step, prior to performing variational assimilation. This step aims at finding a suitable initial value for $\psi_2^0$ (the initial condition of the sublayer stream function, which is included in the control vector). As they have no information on the sublayer flow, they fall back to using a linear combination of $\sf_1^0$ and $0$. In the absence of any model error correction, this might prevent their two-layer model from emulating multilayer dynamics. Indeed, as shown by~\citeA{hurlburt1986}, instantiating a two-layer (primitive) equation model using a one-layer-like condition (for example with an inert bottom layer at instantiation) yields little to no better results than using the equivalent one-layer reduced-gravity primitive equation model.
...
In order to keep a reasonable number of parameters for the expression of $\tilde{\sf}_2$, we propose to emulate it using a reduced-order space.
We express $\tilde{\sf}_2$ over a quite simple space -- namely a partition of unity of space-time Gaussian envelopes --, so that RG-SI performances could not be attributed to the structure of the parameterization rather than the actual error model. This reduced-order space is different from the one used in the implementation of RG-F, the latter being inspired from the methodology of~\citeA{leguillou2025} (see~\ref{appendix:rg-f-implementation} for details).
...
\end{eqnarray}
where $t_i, x_j, y_j$ are the locations of the envelope centers and $\sigma_T, \sigma_X, \sigma_Y$ their standard deviations. Envelopes centers are chosen so that they are evenly spaced (whether in time or space). Details on the determination of the standard deviations can be found in~\ref{appendix:rg-si-parameterization}.
...
This decomposition allows us to represent a smooth field while enabling the emergence of some rather small scale patterns.
...
\subsubsection{Physics-based regularization}\label{sec:reg}
Efficiency of the optimization is further improved by using a physics-based regularization based on an approximate form of the conservation of PV, as introduced in~\cref{sec:PV2reg}.
...
Replacing the usual background state related regularization by this physics-based formulation is inspired by physics-informed neural networks (PINN) methodology~\cite{raissi2019} and PINN-derived data assimilation approaches which have proven the benefits of regularizing using physical constraint~\cite{ugur2025,oh2025}.

Switching to such physics-based regularization prevents from nudging $\tilde{\sf}_2$ towards an \textit{a priori} estimation of the field. It brings more flexibility in the optimization process and allows the model to stir away from the RG framework. Rather than constraining the value of the sublayer stream function itself (as a background regularization on its initial condition would do), such regularization enforces two-layer-like motion. Only does the model need to satisfy a pseudo-advection of PV in the bottom layer, but as for its value it remains unconstrained.

It is worth emphasizing that introducing this regularization does not strictly enforce the conservation of PV in the sublayer. It guides the model towards finding an expression for $\tilde{\sf}_2$ that approaches the conservation of $\tilde{\pv}_2$. As there are no guarantees that $\tilde{\pv}_2$ should be strictly conserved, this formulation allows for some flexibility. Indeed, if improving surface dynamics requires drifting away from the advection of the sublayer (truncated) PV (through the introduction of non-QG patterns in the reconstructed field for example), it can do so. A strictly enforced two-layer QG model would not be able to handle such a situation without the introduction of an external forcing term in the sublayer advection equation.
...
We have considered two observing system simulation experiment (OSSE) validation experiments. In the first one, we have set a double-gyre experiment using a three-layer QG model (QG3L). The reconstruction error from the RG model is mostly explained by the truncation of the QG equations. The second validation experiment uses reference surface fields from the high-resolution, realistic simulation eNATL60 simulation~\cite{brodeau2020}, which is based on NEMO~\cite{madec2024} an includes realistic forcings and bathymetry. This experiment investigates the capability of our model in depicting a flow that present ageostrophic, complex and rich dynamical features.

\subsection{Configuration of OSSEs}

We retrieved ideal initial condition (IC) and boundary conditions (BC) from the surface flow of the reference simulation. Hence, since all tested frameworks (RG0, RG-F, RG-SI) share identical IC and BC, differences in performances only result from model error and its correction. Although IC and BC can be included in the control vector to further extend the scope of this work, their estimation through variational assimilation is already a field of study of its own~\cite{cong1998,zou1995,gunson1996,seiler1993,ayoub2006} and would require additional investigations.
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.95\textwidth]{satellite_track.pdf}
    \end{center}
...
\subsection{3-layer QG double gyre}\label{sec:qg3l}

In the following the reference surface fields are established using a three-layer QG simulation in a double-gyre configuration. Within this framework, we can assess the ability of the RG-SI model in reconstructing top layer dynamics and compare its performances with a RG model with no error correction (RG0) and a RG model whose error is prescribed though an external forcing, RG-F (Reduced Gravity - Forced). Two variants of this setup have been explored. One where model parameters (reference layer depth, reduced gravity values\dots) are the same in the reference QG model and in the RG counterparts. Not only does this configuration confine the error sources to truncation error, it also allows for a fair comparison of $\tilde{\sf}_2$ and $\sf_2$. In the other variant, model parameters are modified to assess the robustness of each of the models when dealing with incorrect baroclinic deformation radius. Since the reference emulates the dynamics over a $\qty{2560}{\kilo\meter}$ by $\qty{5120}{\kilo\meter}$ domain, we have decided to perform data assimilation on subdomains of the said area. We selected two $\qty{640}{\kilo\meter}$ x $\qty{1280}{\kilo\meter}$ zones, Z1, Z2, presenting different dynamical features.

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.7\textwidth]{zones.pdf}
    \end{center}
    \caption{Detail of the 4 zones of the three-layer QG experiment.}
\end{figure}

Z1 is at the core of the eastward jet and presents a surface intensified dynamics whereas on Z2 the barotropic component of the flow is stronger. In both zones, the OSSE follows the same tracks, observing 13243 gridpoints. Number of control parameters for the different models are detailed in~\cref{tab:qg3l-control-vector-size}.
...
Both the reference and the models (RG-SI, RG-F, RG0) were configured with the same time and space resolutions. Regarding stratification parameters, RG0 has been implemented as a one-layer QG model with $g = g_1 g_2/(g_1+g_2)$ so that it matches the parameters of a truncation of the multilayer equations (see~\cref{def:pv-rg}). RG-SI and RG-F have been instantiated so that they share the same deformation radius, namely $\qty{29}{\kilo\meter}$, which corresponds to the baroclinic deformation radius of the two-layer QG model matching the parameters of the reference three-layer QG on the top two-layers. Such value is coherent with typical values of baroclinic deformation radius at mid-latitudes~\cite{chelton1998}. All model parameters are detailed in~\cref{tab:qg3l-model-parameters}.
...
Since the reference simulation is forced by wind to generate a double-gyre circulation, we also include the wind as an external forcing within the advection equations for all models. Denoting $\tau = \transpose{\begin{bmatrix} \tau_x & \tau_y \end{bmatrix}}$ the wind stress tensor, the advection equation rewrites
...
\begin{eqnarray}
    ...
    F_{\mathrm{RG-SI}} & = & f_0^2 \tilde{\mat{A}}_{1,2}\Dt{\tilde{\sf}_2}.
\end{eqnarray}

Both RG-F and RG-SI regularizations are scaled through an hyperparameter ($\gamma$). The value of $\gamma$ has been taken constant over all areas and assimilation periods. Regarding RG-SI, its value has been fitted so that regularization cost function on first assimilation step was around 1\%-1‰ of the observation loss in all areas and periods. Therefore, cost function is dominated by the observation error at first, which is then reduced in priority. Regarding RG-F, the value $\gamma$ has been empirically chosen so that the model performs as best as possible while not displaying non-physical small-scale artefacts in the reconstructed fields (which it tends to do in the absence of regularization).
...
Comparison with other methods shows that RG-SI outperforms both RG0 and RG-F.
Such result highlights that the introduction of an external forcing (the RG-F methodology), although able to mitigate some of the RG-truncation error, is not as effective as the truncation-aware method as introduced in RG-SI.
It is also worth mentioning that, as a comparison, we have tried fitting $\tilde{\sf}_2$ using the reference $\sf_2$ field of the QG3L model. Plugging the resulting parameters into the RG-SI model yields similar results as RG-SI (corresponding figure can be found in~\cref{fig:qg3L-proj-rmses}, averaged nRMSE values over stream function can be found in the RG-Proj column of~\cref{tab:qg3l-nrmses}).
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{qg3l_vorticity_all_32_96_256_384.pdf}
    \end{center}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{qg3l_psi2_rmses.pdf}
    \end{center}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{qg3l_pert_rmses.pdf}
    \end{center}
    \caption{nRMSE on three-layer QG surface stream function (averaged over 12 different assimilation periods each of 20 days) on Z1 (left) and Z2 (right) using perturbed models. Lines represent the nRMSE average and the shaded area the standard deviation. Models are identified by the line color, black for RG0, brown for RG-F and dark blue for RG-SI. A light blue line is used for the RG-SI model with $\alpha=0$ (hence no deformation radius correction).}
\end{figure}

Here again, RG-SI accurately reconstructs the surface flow dynamics. The comparison of error scores of the standard RG-SI model and its counterpart with fixed $\alpha = 0$ shows that the introduction of a single additional parameter into the control vector results in an error reduction of more than 2 (see~\cref{tab:qg3l-pert-nrmses}). $\mathrm{nRMSE}_{\mathrm{vel}}$ scores are not displayed as they do not provide further information to the nRMSE profiles.
...
Relative comparison with the error scores from~\cref{tab:qg3l-nrmses} shows that, overall, RG-SI still performs worse in Z1 in the perturbed framework (absolute error difference is 0.005) and, surprisingly performs better in Z2 (reduction of error by 0.005). We reckon that this difference can be explained by the fact that the dynamics in Z2 is much more barotropic than it is in Z1. Therefore, the model performances in Z2 are not as impacted by the baroclinic radius perturbation as they are in Z1.
Regarding RG-F performances, they are both degraded in the perturbed framework, notably in Z1 (absolute error difference is 0.07) and slightly in Z2 (error difference is 0.01). Nonetheless, perturbed RG-SI still outperform non-perturbed RG-F in reconstructing the surface flow of the three-layer QG model.

\subsection{Validation with realistic OSSE}\label{sec:enatl60}

Using surface fields from the eNATL60 simulation~\cite{brodeau2020}, we monitor RG-SI's reconstruction of a fluid with ageostrophic dynamics. We have focused on a single area of dimension $\qty{680}{\kilo\meter}$ x $\qty{1030}{\kilo\meter}$ centered on $\qty{55}{\degree}$W $\qty{40}{\degree}$N. Located at the east of the US coast, this area contains Gulf Stream meanders that drive its dynamics.
This area, t located at the east of the US coast
...
As the resulting stream function contain small-scale features, it is then smoothed using a Gaussian filter and then downsampled from the high-resolution, curvilinear coordinates of the eNATL60 simulation into the regular, Cartesian grid of the QG models. It is this smoothed, down-scaled field that is used for both initial and boundary conditions of the QG models. The observations are sampled from the non-filtered surface stream function field, using the same OSSE as for the three-layer QG comparison. A total of 10333 pixels are observed during a single assimilation cycle. Control vectors sizes are given in~\cref{tab:enatl60-control-vector-size}.
\begin{table}[h]
    \begin{center}
        \begin{tabular}{lccc}
            & RG-SI & RG-F & RG0 \\
...
The visual comparison of the surface vorticity field demonstrates the ability of the RG-SI method in reconstructing a realistic flow. An example of the vorticity fields is shown in~\cref{fig:enatl60-vorticity}. The overall comparison of the models show that RG-SI is able to reconstruct larger vortices, although its resolution prevents it from representing smaller ones. RG-SI also seems to outperform the other models in locating the larger vortices.
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{enatl60_vorticity_summer.pdf}
    \end{center}
...
We formulated a variational assimilation framework using this model, and we replaced the usual background regularization by a physics-based regularization enforcing subsurface PV conservation. Using OSSE-sampled surface data from a three-layer QG simulation, we reconstructed surface state through the minimization of the resulting cost function. Both nRMSE scores and visual comparison of the surface vorticity fields showed the benefits of using such error parameterization and regularization: stream function nRMSE showed a reduction of error by more than 0.11 when compared to a model corrected by an external forcing. These findings confirm the ability of the method in mitigating truncation error thus highlighting the benefits of refining error parameterization using theoretical knowledge of the dynamics.
Despite being able to provide an estimation for the subsurface stream function, comparison to actual subsurface stream function in the three-layer QG experiment yielded moderate results. These results can be explained by the fact that, when correcting surface advection equation, only the subsurface stream function's material derivative is accessed and is therefore better constrained by observations than the actual field is.
To evaluate our method over richer dynamics we then replicated the same experiment with observations sampled from the eNATL60 simulation. Results analysis has shown that the RG-SI methodology was able to improve surface reconstruction even for non-QG flows yet with a more modest improvement (stream function nRMSE error reduction of around 0.05). Nonetheless, although our method makes a strong assumption on the dynamics of the flow (as it enforces multilayer QG dynamics), such improvement over realistic flow suggests that a significant part of the reconstruction error can be explained by a field advected by the geostrophic flow. However, in the context of a realistic flow, the identification of the parameterized field to the subsurface stream function can be discussed.
 

Despite the improvement made over other reconstruction methods, the error in the eNATL60 experiment remains significant (more than 0.1 for stream function nRMSE and above 0.4 for velocity error). The method we have introduced specifically addresses truncation error but several sources of error are not taken into account. 
For instance, the method does not emulate any submesoscale processes, nor ageostrophic dynamics. Wind forcing and bottom topography influences are also not accounted for by the model. These sources of error can also be parameterized and integrated into the RG-SI framework. 
Increasing the resolution of the model could help to better capture small-scale dynamics, although it would also increase the computational cost of the assimilation.
Additionally, rather than using the QG PV expression to formulate the error term, it might be interesting to dig into the next-order terms of the QG approximation~\cite{muraki1999}. Doing so would restore the proximity with Ertel's potential vorticity~\cite{muller1995} thus favoring the emergence of ageostrophic dynamics. 
Furthermore, since the error parameterization, as formulated in RG-SI, only accounts for a specific source of error, the integration of physical forcing into the model might help better approximate the surface flow. Bottom topography for instance, cannot be taken into account by the RG model (as pointed out by~\citeA{hurlburt1986}) but could be integrated into the RG-SI methodology through appropriate modifications of the regularization term.
...
%
\bibliography{refs} % Replace ``wiley'' with your bibliography's file name.  Do not specify its extension, e.g., .bib.
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{qg3l_all_rmses_.pdf}
    \end{center}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{qg3l_Dt_psi2_rmses.pdf}
    \end{center}
