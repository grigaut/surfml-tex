...
Retrieving a relevant estimation for $\theta^b$ can already be a challenge of itself and should not be overlooked. Indeed, when performing data assimilation using multilayer QG models, it is often difficult to obtain a precise \textit{a priori} estimation for the subsurface flow that can be used as a background term. To overcome this limitation, one could penalize deviation from 0 or from the initial surface state, $\sf_1^0$, but doing so would strongly call back the initial condition toward either a surface intensified or a barotropic flow. Since this background term is solely there to ensure the well-posedness of the minimization problem, the background regularization can be replaced by any loss function that would enforce an \textit{a priori} knowledge on the dynamics. For instance, penalizing of the stream function's second derivatives from deviating from 0 has been proven to be effective in regularizing the assimilation problem as it enforces the smoothness of the fields~\parencite{vogeler1995,cong1995}.

However, this does not replace the need for an actual initial condition to start assimilating from. To overcome this difficulty,~\cite{cong1998} decided to use a preparation step, prior to performing variational assimilation, in order to find the suitable linear combination of $\sf_1^0$ (the initial condition for the surface stream function) and $0$ to use as initial conditions. Still, not only does an appropriate initial condition help the assimilation algorithm converges towards a minimum~\parencite{morrow1995}, it also helps the model in emulating relevant dynamics. Indeed, as shown by~\cite{hurlburt1986}, instantiating a two-layer primitive equation model using a RG-like condition (hence inert bottom layer at instantiation) yields little to no better results than using the equivalent one-layer reduced-gravity primitive equation model. Hence, in the absence of model error correction, improving model's complexity does not suffice to mitigate unaccurate initial conditions.
...
Numerical experiments were conducted using the Grid'5000 tesbed~\parencite{balouek2013}.
