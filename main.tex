...
As a result, the reduced-gravity model as expressed in~\cref{eq:qg-rg} was used for the first time to perform dynamical interpolation~\cite{ubelmann2015,ubelmann2016}. 
...
However, this does not replace the need for an actual initial value $\theta^0$ for $\theta$ to start assimilating from. To overcome this difficulty,~\citeA{cong1998} decided to use a preparation step, prior to performing variational assimilation. This step aims at finding a suitable initial value for $\psi_2^0$ (the initial condition of the sublayer stream function, which is included in the control vector). As they have no information on the sublayer flow, they fall back to using a linear combination of $\sf_1^0$ and $0$. In the absence of any model error correction, this might prevent their two-layer model from emulating multilayer dynamics. Indeed, as shown by~\citeA{hurlburt1986}, instantiating a two-layer (primitive) equation model using a one-layer-like condition (for example with an inert bottom layer at instantiation) yields little to no better results than using the equivalent one-layer reduced-gravity primitive equation model.
